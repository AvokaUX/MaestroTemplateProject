        ...
        app.ng.controller("DockingProgressBarController", ["$scope", "$element", "Form", "Resource", "Validation", "Util", "$timeout",
            function($scope, $element, Form, Resource, Validation, Util, $timeout) {    
              Form.getItem($scope, $element).then(function (item) {
                  ...
                  var disableDocking = Form.view.brands[Form.view.selectedBrand].styleVars['av-disable-docking-progressbar'];
                  ...
                  if (Form.$Pages.visiblePageCount > 1) {
                      $scope.$navContainer.css({"width": "calc(100% + " + $scope.$logo.width() + "px)"});
                      $scope.$navContainer.css({"top": ($scope.$header.outerHeight() + $scope.progressBarHeight) / 2 + "px"});
                  }
                  if ($scope.dp > 0 || disableDocking != "false") {
                      $scope.$watch("data.scrollPosition", function (newValue, oldValue) {
                          ...
                          var relativeMargin = ($(window).width() * 0.01) + 60;
                          if (Form.$Pages.visiblePageCount > 1) {
